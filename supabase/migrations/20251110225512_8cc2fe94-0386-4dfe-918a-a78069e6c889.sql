...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lots' AND policyname = 'Users can view own lots') THEN
    CREATE POLICY "Users can view own lots" ON lots FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lots' AND policyname = 'Users can insert own lots') THEN
    CREATE POLICY "Users can insert own lots" ON lots FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lots' AND policyname = 'Users can update own lots') THEN
    CREATE POLICY "Users can update own lots" ON lots FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'lots' AND policyname = 'Users can delete own lots') THEN
    CREATE POLICY "Users can delete own lots" ON lots FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_show_cards_user_id ON show_cards(user_id);
CREATE INDEX IF NOT EXISTS idx_show_cards_lot_id ON show_cards(lot_id);
CREATE INDEX IF NOT EXISTS idx_show_cards_status ON show_cards(status);
CREATE INDEX IF NOT EXISTS idx_show_cards_player_name ON show_cards(player_name);
CREATE INDEX IF NOT EXISTS idx_show_cards_year ON show_cards(year);
CREATE INDEX IF NOT EXISTS idx_show_cards_player_search ON show_cards USING gin(to_tsvector('english', player_name));

ALTER TABLE show_cards ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'show_cards' AND policyname = 'Users can view own show cards') THEN
    CREATE POLICY "Users can view own show cards" ON show_cards FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'show_cards' AND policyname = 'Users can insert own show cards') THEN
    CREATE POLICY "Users can insert own show cards" ON show_cards FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'show_cards' AND policyname = 'Users can update own show cards') THEN
    CREATE POLICY "Users can update own show cards" ON show_cards FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'show_cards' AND policyname = 'Users can delete own show cards') THEN
    CREATE POLICY "Users can delete own show cards" ON show_cards FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_shows_show_date ON shows(show_date DESC);

ALTER TABLE shows ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'shows' AND policyname = 'Users can view own shows') THEN
    CREATE POLICY "Users can view own shows" ON shows FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'shows' AND policyname = 'Users can insert own shows') THEN
    CREATE POLICY "Users can insert own shows" ON shows FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'shows' AND policyname = 'Users can update own shows') THEN
    CREATE POLICY "Users can update own shows" ON shows FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'shows' AND policyname = 'Users can delete own shows') THEN
...
CREATE INDEX IF NOT EXISTS idx_expenses_show_id ON expenses(show_id);
...
CREATE INDEX IF NOT EXISTS idx_transactions_show_card_id ON transactions(show_card_id);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cash_transactions' AND policyname = 'Users can view own cash transactions') THEN
    CREATE POLICY "Users can view own cash transactions" ON cash_transactions FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cash_transactions' AND policyname = 'Users can insert own cash transactions') THEN
    CREATE POLICY "Users can insert own cash transactions" ON cash_transactions FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'cash_transactions' AND policyname = 'Users can delete own cash transactions') THEN
    CREATE POLICY "Users can delete own cash transactions" ON cash_transactions FOR DELETE USING (auth.uid() = user_id);
  END IF;
END $$;
...
DROP TRIGGER IF EXISTS update_show_cards_updated_at ON show_cards;
